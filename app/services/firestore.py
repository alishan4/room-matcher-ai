...
def fetch_all_profiles() -> List[Dict]:
    if not USE_FIRESTORE:
        ...
        raw = _local_json("synthetic_roommate_profiles_pakistan_400.json")
...
def fetch_all_listings() -> List[Dict]:
    if not USE_FIRESTORE:
        return _local_json("housing_listings_pakistan_400.json")
...
class LocalDB:
    """Compat wrapper some modules use; returns normalized profiles."""
    def __init__(self):
